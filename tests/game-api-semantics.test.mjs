...
test('every live POST route rejects a cross-site request before any mutation work', () => {
  const apiRoot = path.join(root, 'src/app/api');
  const routeFiles = [];
  const visit = (directory) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
  ...
  for (const routeFile of routeFiles) {
    const source = fs.readFileSync(routeFile, 'utf8');
    if (!/export async function POST/.test(source)) continue;
    assert.match(source, /isSameOriginRequest\(request\)/, routeFile);
  }
});
...
test('event enrollment writer is a same-origin, claims-checked RPC boundary without direct table access', () => {
  ...
  for (const [source, legacy, hashArguments, operation] of [[enrollmentWrapper, 'enroll_linked_roster_entry_in_event', /'enroll_linked_roster_entry_in_event', p_tournament_id::text, p_event_id::text,\s*p_roster_entry_id::text, p_idempotency_key::text/, 'enroll_linked_roster_entry_in_event']]) {
    ...
    assert.match(source, /v_hash := encode\(extensions\.digest\(convert_to\(jsonb_build_array\([\s\S]*?\)::text, 'utf8'\), 'sha256'\), 'hex'\);/);
    assert.match(source, /select o\.response_payload into v_stored from app\.operation_receipts o/);
    assert.match(source, /o\.actor_profile_id = auth\.uid\(\)/);
    assert.match(source, /o\.tournament_id = p_tournament_id/);
    assert.match(source, new RegExp(`o\\.operation_type = '${operation}'`));
    assert.match(source, /o\.target_id = p_roster_entry_id/);
    assert.match(source, /o\.client_operation_id = p_idempotency_key/);
    assert.match(source, /o\.request_hash = v_hash/);
    const performIndex = source.indexOf(`perform public.${legacy}`);
    const queryRoleIndex = source.lastIndexOf("r.role in ('director', 'co_director')");
    assert.ok(queryRoleIndex > performIndex, 'receipt query must recheck the current role after the legacy writer');
    assert.match(source, /and exists \(select 1 from app\.tournament_roles r where r\.tournament_id = p_tournament_id\s*and r\.profile_id = auth\.uid\(\) and r\.role in \('director', 'co_director'\)\)/);
    assert.match(source, /if v_stored is null then return null; end if/);
    assert.match(source, /return v_stored \|\| jsonb_build_object/);
    assert.doesNotMatch(source, /return v_result|coalesce\(v_stored/);
    const returns = [...source.matchAll(/\breturn\s+([\s\S]*?);/g)].map((match) => match[1].replace(/\s+/g, ' ').trim());
    assert.deepEqual(returns, ['null', 'null', "v_stored || jsonb_build_object('tournamentId', p_tournament_id, 'operationId', p_idempotency_key)"]);
  }
...
test('correction API handlers validate request shapes and discriminate accepted rejection from operation failure', () => {
  ...
  assert.match(proposal, /createRule12Correction/);
  ...
  assert.match(correctionRelease, /export function rule12CorrectionEnabled\(\)/);
  assert.match(correctionRelease, /return true/);
  assert.doesNotMatch(correctionRelease, /ACC_RULE12_CORRECTION_ENABLED/);
  ...
  assert.match(proposal, /ruleCase/);
  assert.match(proposal, /claimA/);
  assert.match(proposal, /body\.reason\.length > 500/);
  assert.match(review, /reviewRule12Correction/);
  assert.match(review, /\["approve", "reject"\]/);
  assert.match(review, /createServerOnlyAdminClient/);
  assert.match(reconciliation, /get_rule12_correction_reconciliation_v1/);
...
test('director correction policy workspace is scoped, append-only, and retry-safe', () => {
  ...
  assert.match(policyDal, /get_rule12_correction_policy_v1/);
  ...
  assert.match(policyRoute, /rule12CorrectionEnabled\(\)/);
  assert.match(policyRoute, /configure_rule12_correction_policy_v1/);
  ...
  assert.match(reconciliationRoute, /get_rule12_correction_policy_reconciliation_v1/);
...
test('manual roster payments are immutable director-only evidence, never enrollment or paid-in-full authority', async () => {
  ...
  const validPaymentWorkspace = { rosterEntries: [{ rosterEntryId: '00000000-0000-4000-8000-000000000002', displayName: 'Sample Player', plannedPaymentMethod: 'cash', paymentVersion: 1, paymentState: 'received', currentReceiptEventId: event.paymentEventId, history: [event] }] };
  assert.equal(payment.isPaymentWorkspace(validPaymentWorkspace), true);
  assert.equal(payment.isPaymentWorkspace({ rosterEntries: [{ ...validPaymentWorkspace.rosterEntries[0], plannedPaymentMethod: 'venmo' }] }), false);
  ...
  assert.equal(payment.isPaymentRecordRequest(paymentRecord), true); assert.equal(payment.isPaymentRecordRequest({ ...paymentRecord, paymentMethod: 'other' }), false); assert.equal(payment.isPaymentRecordRequest({ ...paymentRecord, amountMinor: 0 }), false); assert.equal(payment.isPaymentRecordRequest({ ...paymentRecord, amountMinor: 2147483648 }), false); assert.equal(payment.isPaymentRecordRequest({ ...paymentRecord, paymentReceivedAt: '2026-09-09' }), false); assert.equal(payment.isPaymentRecordRequest({ ...paymentRecord, note: 'x'.repeat(501) }), false);
  ...
  assert.equal(payment.isRecordedPayment({ status: 'payment_recorded', paymentEventId: event.paymentEventId, rosterEntryId: paymentRecord.rosterEntryId, paymentVersion: 1, paymentState: 'received', paymentRecorded: true, paidInFull: false, reconciled: false, internal_detail: 'must not reach the browser' }, paymentRecord), false);
  ...
  assert.equal(payment.isVoidedPayment({ status: 'payment_voided', paymentEventId: '00000000-0000-4000-8000-000000000012', voidedPaymentEventId: event.paymentEventId, rosterEntryId: paymentRecord.rosterEntryId, paymentVersion: 2, paymentState: 'voided', paymentRecorded: false, paidInFull: false, reconciled: false, internal_detail: 'must not reach the browser' }, paymentVoid), false);
  ...
  assert.equal(payment.isRejectedPayment({ status: 'rejected', code: 'stale_payment_history', rosterEntryId: paymentRecord.rosterEntryId, paymentVersion: 1 }, paymentRecord.rosterEntryId, 'record_manual_roster_payment'), false);
  assert.equal(payment.isRejectedPayment({ status: 'rejected', code: 'active_payment_receipt_exists', rosterEntryId: paymentRecord.rosterEntryId }, paymentRecord.rosterEntryId, 'record_manual_roster_payment'), true);
  assert.equal(payment.isRejectedPayment({ status: 'rejected', code: 'current_payment_receipt_required', rosterEntryId: paymentRecord.rosterEntryId }, paymentRecord.rosterEntryId, 'record_manual_roster_payment'), false);
...
test('protected correction workspace reads only the scoped RPC and never direct tables', () => {
  ...
  assert.match(dal, /get_rule12_correction_workspace_v2/);
  assert.match(dal, /proposalCandidates/);
  assert.match(dal, /pendingReviews/);
  assert.doesNotMatch(dal, /\.from\(/);
  assert.match(client, /crypto\.randomUUID\(\)/);
  assert.match(client, /Rulebook disposition/);
  assert.match(client, /apparentQualifier/);
  assert.match(client, /affectedParticipantId/);
  assert.match(client, /router\.refresh\(\)/);
  assert.match(client, /response\.status<500/);
  assert.match(client, /Retry exact saved correction/);
  assert.match(client, /Review and save correction/);
