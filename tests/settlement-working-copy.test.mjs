...
function fixtures() {
  ...
  const workspace = { tournamentId, eventId, qualificationResultVersionId, currencyCode: "USD", currentVersion: 1, qualifierChoices: qualifiers.map(({ participantId, displayName, qualificationRank }) => ({ participantId, displayName, qualificationRank })), configuredQPools: [], playoffResult: { playoffResultVersionId, version: 1, recordedAt: "2026-09-11T00:30:00Z", placements: [{ participantId: participants[1], displayName: "Player Two", placement: 1 }, { participantId: participants[0], displayName: "Player One", placement: 2 }] }, capabilities: { publication: false, approval: false, officialExport: false, mrpTranscription: true, mrpCalculation: false }, draft: { settlementDraftId: "60000000-0000-4000-8000-000000000001", qualificationResultVersionId, playoffResultVersionId, version: 1, createdAt: "2026-09-11T01:00:00Z", createdBy: "Director", placements: [{ participantId: participants[1], placement: 1, prizeAmountMinor: 10000 }, { participantId: participants[0], placement: 2, prizeAmountMinor: 5000 }], awards: [{ participantId: participants[1], awardType: "other", qPoolSlot: null, amountMinor: 2500, note: "+formula" }], mrpClaims: [{ participantId: participants[1], mrpPoints: 0, evidenceNote: "Director transcription from dated ACC worksheet" }], serverTotals: { currencyCode: "USD", activePaymentReceiptCount: 8, activePaymentReceiptTotalMinor: 20000, activeExpenseCount: 1, activeExpenseTotalMinor: 5000, netCashPositionMinor: 15000 }, reconciled: false, blockers } };
...
test("working copy separates playoff claims from locked qualification order and High Non-Qualifier", () => {
  ...
  assert.ok(csv);
  assert.match(csv, /PROVISIONAL — UNRECONCILED — NOT APPROVED — NOT AN ACC SUBMISSION/);
  assert.match(csv, /"Schema","settlement-working-copy-v3"/);
  assert.match(csv, /"Canonical source SHA-256","[0-9a-f]{64}"/);
  ...
  assert.ok(csv.indexOf('"High Non-Qualifier","50000000-0000-4000-8000-000000000001","Casey Kim"') > csv.indexOf('"2","40000000-0000-4000-8000-000000000002","Jordan Patel"'));
  assert.match(csv, /"40000000-0000-4000-8000-000000000002","Jordan Patel","ENTERED","0","Director transcription from dated ACC worksheet"/);
  assert.match(csv, /"40000000-0000-4000-8000-000000000001","'=unsafe player","MISSING","",""/);
  assert.match(csv, /This private working copy is unreconciled, not approved, and not an ACC submission/);
...
test("working copy requires an exact qualification version and neutralizes spreadsheet formulas", () => {
  const { workspace, qualification } = fixtures();
  const csv = buildSettlementWorkingCopyCsv(workspace, qualification);
  assert.equal(csv, buildSettlementWorkingCopyCsv(workspace, qualification));
  ...
  assert.equal(buildSettlementWorkingCopyCsv({ ...workspace, draft: { ...workspace.draft, qualificationResultVersionId: tournamentId } }, qualification), null);
  ...
  assert.equal(buildSettlementWorkingCopyCsv({ ...workspace, playoffResult: { ...workspace.playoffResult, playoffResultVersionId: tournamentId } }, qualification), null);
  assert.equal(buildSettlementWorkingCopyCsv({ ...workspace, playoffResult: null }, qualification), null);
  assert.equal(buildSettlementWorkingCopyCsv({ ...workspace, draft: { ...workspace.draft, playoffResultVersionId: null } }, qualification), null);
...
test("working-copy route is authenticated, server-derived, private, and fail-closed", () => {
  ...
  assert.match(route, /const csv = buildSettlementWorkingCopyCsv\(workspace, qualification\);[\s\S]*if \(!csv\) return apiJson\(\{ error: "settlement_draft_required" \}, \{ status: 409 \}\)/);
  assert.match(route, /private, no-store/);
  assert.match(route, /x-content-type-options/);
  assert.match(route, /settlement-working-copy-v3/);
