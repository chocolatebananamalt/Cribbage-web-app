...
test('registration page clears its fragment before hydration and uses no browser persistence', () => {
  ...
  assert.match(form, /credentialRef\.current = null/); assert.match(form, /window\.addEventListener\("pagehide", onPageHide\)/); assert.match(form, /requestRef\.current\?\.abort\(\)/); assert.match(form, /dropCredential\(\);/); assert.match(form, /signal: controller\.signal/);
