-- Repair proposal handling after adding the immutable event publication guard.
...
declare
  ...
  v_hash text; v_legacy_hash text; v_existing app.operation_receipts%rowtype; v_policy app.correction_policy_versions%rowtype; v_tournament_status text; v_publication_state text; v_sequence integer; v_receipt uuid; v_response jsonb; v_error text; v_code text;
begin
  begin
  ...
  select state into v_publication_state from app.event_publication_states where event_id = v_game.event_id and tournament_id = v_game.tournament_id for update;
