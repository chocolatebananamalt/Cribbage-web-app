-- Restore replay compatibility in the pilot after the initial canonical
-- fingerprint repair. Existing receipts are immutable, so comparison accepts
-- the exact legacy fingerprint only for an already-recorded operation.
...
declare
  ...
  v_hash text; v_legacy_hash text; v_existing app.operation_receipts%rowtype; v_sequence integer; v_receipt uuid; v_response jsonb; v_error text; v_code text;
begin
  begin
