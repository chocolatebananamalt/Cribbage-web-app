-- A fragment credential is link-id.secret, so the application server must
-- generate the opaque link ID before it derives the salted digest.  The
-- original v2 writer generated that ID inside the database, which made a
-- future credential impossible to bind to the stored digest.
...
drop function public.issue_registration_link_v2(uuid, uuid, bytea, bytea, timestamptz, integer, integer, uuid);
drop function public.rotate_registration_link_v2(uuid, uuid, bytea, bytea, timestamptz, integer, integer, uuid);
drop function app.write_registration_link_v2(text, uuid, uuid, bytea, bytea, timestamptz, integer, integer, uuid);

create function app.write_registration_link_v2(
...
create function public.issue_registration_link_v2(
  p_actor_id uuid, p_tournament_id uuid, p_link_id uuid, p_salt bytea, p_digest bytea, p_expires_at timestamptz,
  p_max_claims integer, p_max_claims_per_hour integer, p_operation_id uuid
) returns jsonb language sql security definer set search_path = '' as $$
  select app.write_registration_link_v2('issue', p_actor_id, p_tournament_id, p_link_id, p_salt, p_digest, p_expires_at, p_max_claims, p_max_claims_per_hour, p_operation_id)
$$;
...
create function public.rotate_registration_link_v2(
  ...
  select app.write_registration_link_v2('rotate', p_actor_id, p_tournament_id, p_link_id, p_salt, p_digest, p_expires_at, p_max_claims, p_max_claims_per_hour, p_operation_id)
$$;
...
grant execute on function public.rotate_registration_link_v2(uuid, uuid, uuid, bytea, bytea, timestamptz, integer, integer, uuid) to service_role;
