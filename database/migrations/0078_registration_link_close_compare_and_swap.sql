-- Close must be compare-and-swap, not a broad tournament action. A stale
-- director screen may only close the exact head it read.

revoke all on function public.close_registration_link_v2(uuid, uuid, uuid) from public, anon, authenticated;
drop function public.close_registration_link_v2(uuid, uuid, uuid);

create function public.close_registration_link_v2(
  p_actor_id uuid,
...
begin
  ...
  if v_tournament_status not in ('draft', 'open') or v_registration_status <> 'open'
    or not found or v_head.state <> 'open' or v_head.registration_link_id <> p_expected_link_id
