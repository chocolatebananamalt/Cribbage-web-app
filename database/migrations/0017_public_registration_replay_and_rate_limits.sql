-- Repair the already-applied public-registration pilot and add server-enforced
-- replay equality and bounded anonymous intake.
alter table app.tournament_registration_links
  add column if not exists max_claims integer not null default 500 check (max_claims between 1 and 2000),
  add column if not exists max_claims_per_hour integer not null default 120 check (max_claims_per_hour between 1 and 1000);

-- Temporarily remove immutability only while repairing legacy rows. The
-- trigger is restored before this migration completes.
drop trigger if exists registration_claims_immutable on app.registration_claims;

alter table app.registration_claims add column if not exists request_fingerprint text;
...
alter table app.registration_claims alter column request_fingerprint set not null;
do $$
begin
  if not exists (
    select 1 from pg_catalog.pg_constraint
    where conrelid = 'app.registration_claims'::regclass
      and conname = 'registration_claims_request_fingerprint_check'
  ) then
    alter table app.registration_claims
      add constraint registration_claims_request_fingerprint_check
      check (length(request_fingerprint) = 64);
  end if;
end;
$$;
create index if not exists registration_claims_link_submitted_at_idx
