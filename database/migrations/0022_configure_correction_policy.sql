...
declare
  ...
  v_receipt uuid; v_response jsonb; v_error text; v_code text;
begin
  begin
  ...
  if not found then raise exception using errcode = 'P0001', message = 'tournament is not configurable'; end if;
  ...
  exception when sqlstate 'P0001' then
    ...
    return jsonb_build_object('status','rejected','code',v_code,'tournament_id',p_tournament_id);
