-- A director can reconcile an ambiguous one-time link issuance without ever
-- reading bearer, salt, digest, claim, or player data.

create function public.get_registration_link_state_v2(
  p_actor_id uuid,
...
begin
  ...
  select * into v_head
  from app.tournament_registration_link_heads
  where tournament_id = p_tournament_id;
  if not found then return jsonb_build_object('status', 'none'); end if;
  select * into v_link
  from app.tournament_registration_links
  ...
  v_state := case
    when v_head.state = 'open' and v_link.lifecycle_state = 'issued' and v_link.expires_at <= now() then 'expired'
    else v_head.state
  end;
