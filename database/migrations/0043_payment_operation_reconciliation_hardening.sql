-- Exact recovery must never conflate record and void receipts that share a
-- roster target. Bind reconciliation to the current actor, operation kind, and
-- canonical request hash as well as tournament/target/idempotency scope.
create or replace function public.get_roster_payment_operation_reconciliation(
  ...
  p_request_hash text, p_idempotency_key uuid
  ...
  select case when auth.uid() is not null and p_tournament_id is not null
    ...
    and p_request_hash is not null and length(p_request_hash) = 64
  ...
  then (select o.response_payload from app.operation_receipts o
    where o.actor_profile_id=auth.uid() and o.tournament_id=p_tournament_id
      ...
      and o.operation_type=p_operation_type and o.request_hash=p_request_hash limit 1)
  else null end
$$;
revoke all on function public.get_roster_payment_operation_reconciliation(uuid,uuid,text,text,uuid) from public, anon;
grant execute on function public.get_roster_payment_operation_reconciliation(uuid,uuid,text,text,uuid) to authenticated;
