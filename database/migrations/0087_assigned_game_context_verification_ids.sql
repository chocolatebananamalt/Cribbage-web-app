-- An assigned player's permanent verification ID is the immutable initial
-- Table/Seat value published after registration closes. Expose only each
-- game's two assigned IDs through the existing assignment-scoped read; never
-- expose roster IDs, profile IDs, or the seating workspace.
