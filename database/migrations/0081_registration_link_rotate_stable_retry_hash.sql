-- A retry creates fresh server-side credential material. Its durable request
-- identity must therefore cover only the stable director intent, not the new
-- link ID, salt, or digest. Otherwise an exact retry becomes a false conflict
-- and can never reach the safe credential-unavailable reconciliation path.
...
create or replace function public.rotate_registration_link_v2(
  ...
  p_link_id uuid, p_salt bytea, p_digest bytea, p_expires_at timestamptz,
...
declare
  ...
  v_head_found boolean := false; v_previous_found boolean := false;
...
begin
  ...
  if p_actor_id is null or p_tournament_id is null or p_expected_link_id is null
    or p_expected_version is null or p_expected_version < 1 or p_link_id is null or p_operation_id is null
  ...
  v_hash := encode(extensions.digest(convert_to(jsonb_build_array(
    ...
    p_expected_link_id::text, p_expected_version, p_expires_at,
    p_max_claims, p_max_claims_per_hour, p_operation_id::text
  ...
  if found then
    if v_existing.request_hash <> v_hash or v_existing.operation_type <> 'registration_link_rotate_v3' then
      insert into app.registration_link_operation_conflicts(actor_profile_id, tournament_id, attempted_operation_id, attempted_request_hash, prior_receipt_id, prior_receipt_tournament_id, reason_code)
      values (p_actor_id, p_tournament_id, p_operation_id, v_hash,
        case when v_existing.tournament_id = p_tournament_id then v_existing.id else null end,
        case when v_existing.tournament_id = p_tournament_id then v_existing.tournament_id else null end, 'idempotency_conflict');
  ...
  v_head_found := found;
  select * into v_previous from app.tournament_registration_links where id = p_expected_link_id and tournament_id = p_tournament_id for update;
  ...
  if v_tournament_status not in ('draft', 'open') or v_registration_status <> 'open'
    ...
    or v_head.registration_link_id <> p_expected_link_id or v_head.version <> p_expected_version
    or v_previous.lifecycle_state <> 'issued' or not v_previous.enabled or v_previous.expires_at <= now() then
    ...
    values (p_actor_id, p_tournament_id, 'registration_link_rotate_v3', p_expected_link_id, v_hash, p_operation_id, 'rejected', v_response, now());
    return v_response;
  end if;
  update app.tournament_registration_links set lifecycle_state = 'retired', enabled = false, disabled_at = now() where id = v_previous.id;
  insert into app.tournament_registration_links(id, tournament_id, token_hash, token_version, token_salt, token_digest, lifecycle_state, issued_at, expires_at, created_by_profile_id, enabled, max_claims, max_claims_per_hour)
  values (p_link_id, p_tournament_id, null, 2, p_salt, p_digest, 'issued', now(), p_expires_at, p_actor_id, true, p_max_claims, p_max_claims_per_hour);
  update app.tournament_registration_link_heads set registration_link_id = p_link_id, state = 'open', version = version + 1, updated_at = now() where tournament_id = p_tournament_id;
  v_response := jsonb_build_object('status', 'rotated', 'linkId', p_link_id, 'state', 'open', 'expiresAt', p_expires_at, 'version', p_expected_version + 1);
  insert into app.operation_receipts(actor_profile_id, tournament_id, operation_type, target_id, request_hash, client_operation_id, outcome, response_payload, applied_at)
  values (p_actor_id, p_tournament_id, 'registration_link_rotate_v3', p_link_id, v_hash, p_operation_id, 'accepted', v_response, now()) returning id into v_receipt_id;
