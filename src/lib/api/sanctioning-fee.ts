...
export type SanctioningFeeRateOverrideRequest = { eventKind: "main" | "consolation"; rateCents: number; reason: string; accReference: string; idempotencyKey: string };

export function isSanctioningFeeRateOverrideRequest(value: unknown): value is SanctioningFeeRateOverrideRequest {
  if (!value || typeof value !== "object" || Array.isArray(value) || !own(value, ["eventKind", "rateCents", "reason", "accReference", "idempotencyKey"])) return false;
  ...
  return (request.eventKind === "main" || request.eventKind === "consolation")
    ...
    && text(request.reason, 1000) && text(request.accReference, 1000) && isUuid(request.idempotencyKey);
}
