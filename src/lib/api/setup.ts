...
import { isTournamentStateTerritory } from "../tournament-timezones.ts";
...
const rootKeys = ["tournamentName", "city", "venue", "venueName", "venueStreet", "venuePostalCode", "stateTerritory", "startsAt", "endsAt", "timezone", "tournamentDirectorPublicName", "tournamentDirectorFirstName", "tournamentDirectorLastName", "tournamentContactPhone", "tournamentContactEmail", "tournamentMailingAddress", "mainSanctioningFeeRateCents", "consolationSanctioningFeeRateCents", "mainSanctioningFeeOverrideReason", "mainSanctioningFeeOverrideReference", "consolationSanctioningFeeOverrideReason", "consolationSanctioningFeeOverrideReference", "officials", "events"];
...
const contactPhone = (v: unknown) => typeof v === "string" && v.trim().length <= 40 && (!v.trim() || (v.match(/\d/g)?.length ?? 0) >= 7);
const contactEmail = (v: unknown) => typeof v === "string" && v.trim().length <= 320 && (!v.trim() || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim()));
...
export function isSetupSaveRequest(value: unknown): value is SetupSaveRequest {
  ...
  const venueProjection = typeof x.venueName === "string" && typeof x.venueStreet === "string" && typeof x.venuePostalCode === "string" ? `${x.venueName.trim()} — ${x.venueStreet.trim()} · ${x.venuePostalCode.trim()}` : null;
  const directorProjection = typeof x.tournamentDirectorFirstName === "string" && typeof x.tournamentDirectorLastName === "string" ? `${x.tournamentDirectorFirstName.trim()} ${x.tournamentDirectorLastName.trim()}` : null;
  return text(x.tournamentName, 200, true) && text(x.city, 160, true) && text(x.venueName, 160, true) && text(x.venueStreet, 240, true) && text(x.venuePostalCode, 20, true) && text(x.venue, 240, true) && x.venue === venueProjection && isTournamentStateTerritory(x.stateTerritory) && localTime(x.startsAt) && localTime(x.endsAt) && text(x.timezone, 128, true) && text(x.tournamentDirectorFirstName, 80, true) && text(x.tournamentDirectorLastName, 80, true) && typeof directorName === "string" && text(directorName, 160, true) && directorName === directorProjection && !["tournament participant", "tournament director", "primary director"].includes(directorName.trim().toLowerCase()) && contactPhone(x.tournamentContactPhone) && contactEmail(x.tournamentContactEmail) && text(x.tournamentMailingAddress, 500) && ratesAreValid
...
const currentKeys = ["revisionId", "version", "tournamentName", "city", "venue", "venueName", "venueStreet", "venuePostalCode", "stateTerritory", "startsAt", "endsAt", "timezone", "tournamentDirectorPublicName", "tournamentDirectorFirstName", "tournamentDirectorLastName", "tournamentContactPhone", "tournamentContactEmail", "tournamentMailingAddress", "mainSanctioningFeeRateCents", "consolationSanctioningFeeRateCents", "mainSanctioningFeeOverrideReason", "mainSanctioningFeeOverrideReference", "consolationSanctioningFeeOverrideReason", "consolationSanctioningFeeOverrideReference", "createdAt", "officials", "events"];
...
export function isSetupWorkspace(value: unknown): value is SetupWorkspace {
  ...
  return history.length > 0 && history[0].version === current.version && history.every((item, index) => index === 0 || item.version === (history[index - 1].version as number) - 1)
    ...
    && text(current.tournamentName, 200, true) && text(current.city, 160, true) && text(current.venue, 240, true) && text(current.venueName, 160) && text(current.venueStreet, 240) && text(current.venuePostalCode, 20) && text(current.stateTerritory, 80)
    && typeof current.startsAt === "string" && typeof current.endsAt === "string" && text(current.timezone, 128, true)
    && text(current.tournamentDirectorPublicName, 160) && text(current.tournamentDirectorFirstName, 80) && text(current.tournamentDirectorLastName, 80) && text(current.tournamentContactPhone, 40) && text(current.tournamentContactEmail, 320) && text(current.tournamentMailingAddress, 500)
