...
export async function buildTeamResultsPdf(tournamentName:string,report:TeamResults){
 ...
 const line=(text:string,strong=false)=>{if(y<52){footer();page=document.addPage([612,792]);y=748;}page.drawText(clip(text,105),{x:36,y,size:strong?14:10,font:strong?bold:regular,color:rgb(.08,.12,.18)});y-=strong?22:15;};
