...
export async function buildSidePoolReportPdf(workspace: SidePoolWorkspace, eventId?: string) {
  ...
  const line = (text: string, strong = false) => { if (y < 50) { footer(); page=document.addPage([612,792]); y=750; } page.drawText(clip(text, 110), { x: 36, y, size: strong ? 14 : 10, font: strong ? bold : regular, color: rgb(.08,.12,.18) }); y -= strong ? 22 : 15; };
