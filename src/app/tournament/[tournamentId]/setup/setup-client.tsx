...
function tournamentDetailIssues(payload: SetupPayload) {
  ...
  const requiredText: Array<[string, string]> = [
    ...
    ["City", payload.city],
    ["Venue name and address", payload.venue],
    ["Tournament Director name", payload.tournamentDirectorPublicName],
  ];
  for (const [label, value] of requiredText) if (!value.trim()) issues.push(label);
  if (["tournament participant", "tournament director", "primary director"].includes(payload.tournamentDirectorPublicName.trim().toLowerCase()) && !issues.includes("Tournament Director name")) issues.push("Tournament Director name");
  ...
  if ((payload.tournamentContactPhone.match(/\d/g)?.length ?? 0) < 7) issues.push("Tournament contact phone");
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.tournamentContactEmail.trim())) issues.push("Tournament contact email");
  return issues;
}
...
export default function SetupClient({ actorId, tournamentId }: { actorId: string; tournamentId: string }) {
  ...
  return <section className="policy-settings setup-workspace"><h2>Tournament details</h2><p className="auth-note">Primary director: {primaryDirectorName}</p>
    <fieldset disabled={busy || !!pending || activated}><div className="setup-grid">
      ...
      <label>Venue name and address<input required maxLength={240} value={payload.venue} onChange={(e) => setPayload({ ...payload, venue: e.target.value })} /></label>
