...
function SanctioningFeePanel({ payload, runningTotalCents, lastUpdatedAt, refreshing, busy, canAdjust, onRefresh, onApply }: { payload: SetupPayload; runningTotalCents: number; lastUpdatedAt: number | null; refreshing: boolean; busy: boolean; canAdjust: boolean; onRefresh: () => void; onApply: (request: SanctioningFeeRateOverrideRequest) => void }) {
  ...
  function save() { if (!eventKind || busy || !reason.trim() || editingRate === storedRate) return; onApply({ eventKind, rateCents: editingRate, reason: reason.trim(), idempotencyKey: crypto.randomUUID() }); setEventKind(null); setReason(""); }
  return <section className="setup-subsection setup-wide sanctioning-fee-panel" aria-labelledby="sanctioning-fee-title"><h3 id="sanctioning-fee-title">ACC Sanctioning Fee Running Total: ${formatUsdInput(runningTotalCents)}</h3><p className="field-help">Calculated from the number of eligible Main and Consolation participants. It is not a player entry fee. Total is recorded as an official snapshot when each event starts.</p><div className="sanctioning-fee-status"><p className="field-help" aria-live="polite">Last updated: {lastUpdatedAt ? new Intl.DateTimeFormat(undefined, { hour: "numeric", minute: "2-digit", second: "2-digit" }).format(new Date(lastUpdatedAt)) : "Loading…"}</p><button type="button" className="secondary" disabled={refreshing} onClick={onRefresh}>{refreshing ? "Refreshing total…" : "Refresh total"}</button></div><div className="setup-grid sanctioning-fee-rate-inputs"><div><RateInput key={`main-sanctioning-rate-${displayedMain}`} label="Main rate per person" value={displayedMain} disabled={busy || eventKind !== "main"} onChange={setMainRateCents} /><button type="button" className="secondary" disabled={busy || !canAdjust || (eventKind !== null && eventKind !== "main")} onClick={() => eventKind === "main" ? save() : begin("main")}>{eventKind === "main" ? "Save Adjusted Main Rate" : "Adjust Main rate"}</button></div><div><RateInput key={`consolation-sanctioning-rate-${displayedConsolation}`} label="Consolation rate per person" value={displayedConsolation} disabled={busy || eventKind !== "consolation"} onChange={setConsolationRateCents} /><button type="button" className="secondary" disabled={busy || !canAdjust || (eventKind !== null && eventKind !== "consolation")} onClick={() => eventKind === "consolation" ? save() : begin("consolation")}>{eventKind === "consolation" ? "Save Adjusted Consolation Rate" : "Adjust Consolation rate"}</button></div></div>{eventKind ? <label><span className="required-label">Reason (<span className="required-field" aria-hidden="true">*</span>required)</span><span className="sr-only">required</span><input required aria-required="true" maxLength={1000} value={reason} onChange={(event) => setReason(event.target.value)} /></label> : null}<p className="field-help">Only adjust with ACC Board approval. Before Start Play only. This creates an immutable, reasoned rate-change record; it does not change player receipts.</p></section>;
}
