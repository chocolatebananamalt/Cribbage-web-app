...
function storePending(key: string, value: PendingSave | PendingActivation) { try { sessionStorage.setItem(key, JSON.stringify(value)); return true; } catch { return false; } }
...
export default function SetupClient({ actorId, tournamentId, activationEnabled }: { actorId: string; tournamentId: string; activationEnabled: boolean }) {
  const storageKey = `tournament-setup:${actorId}:${tournamentId}`, activationStorageKey = `tournament-activation:${actorId}:${tournamentId}`, inFlight = useRef(false);
