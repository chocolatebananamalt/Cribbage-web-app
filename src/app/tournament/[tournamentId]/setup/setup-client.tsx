...
export default function SetupClient({ actorId, tournamentId }: { actorId: string; tournamentId: string }) {
  ...
  const [amendmentEvents, setAmendmentEvents] = useState<SetupEvent[]>([]), [pendingAmendment, setPendingAmendment] = useState<PendingAmendment | null>(null), [primaryDirectorName, setPrimaryDirectorName] = useState(""), [activationStatusAvailable, setActivationStatusAvailable] = useState(true), [finalizationPromptOpen, setFinalizationPromptOpen] = useState(false), [sanctioningFee, setSanctioningFee] = useState<{ mainEligibleParticipantCount: number; consolationEligibleParticipantCount: number } | null>(null), [rateOverrideBusy, setRateOverrideBusy] = useState(false), [teamUpgradeBusy, setTeamUpgradeBusy] = useState(false);
  ...
  return <section className="policy-settings setup-workspace"><h2>Tournament details</h2><p className="auth-note">Primary director: {primaryDirectorName}</p>
    <fieldset disabled={busy || !!pending || activated}><div className="setup-grid">
      ...
      <label>Tournament contact phone (<span className="required-field" aria-hidden="true">*</span><span aria-hidden="true">required</span><span className="sr-only">required</span>)<input required aria-required="true" type="tel" inputMode="tel" autoComplete="tel" maxLength={40} value={payload.tournamentContactPhone} onChange={(e) => setPayload({ ...payload, tournamentContactPhone: e.target.value })} /></label>
      <label>Tournament contact email (<span className="required-field" aria-hidden="true">*</span><span aria-hidden="true">required</span><span className="sr-only">required</span>)<input required aria-required="true" type="email" autoComplete="email" maxLength={320} value={payload.tournamentContactEmail} onChange={(e) => setPayload({ ...payload, tournamentContactEmail: e.target.value })} /></label>
      ...
      <section className="setup-wide setup-subsection" aria-labelledby="sanctioning-fee-title"><h3 id="sanctioning-fee-title">ACC Sanctioning Fee Running Total: ${formatUsdInput(sanctioningFeeRunningTotalCents)}</h3><p className="field-help">Calculated from the number of eligible Main and Consolation participants. It is not a player entry fee. Total is recorded as an official snapshot when each event starts.</p><div className="setup-grid"><RateInput key={`main-sanctioning-rate-${payload.mainSanctioningFeeRateCents}`} label="Main rate per person" value={payload.mainSanctioningFeeRateCents} onChange={(value) => setPayload({ ...payload, mainSanctioningFeeRateCents: value })} /><RateInput key={`consolation-sanctioning-rate-${payload.consolationSanctioningFeeRateCents}`} label="Consolation rate per person" value={payload.consolationSanctioningFeeRateCents} onChange={(value) => setPayload({ ...payload, consolationSanctioningFeeRateCents: value })} /></div>{requiresSanctioningFeeOverrideEvidence("main", payload.mainSanctioningFeeRateCents) ? <><label>Main override reason<input required maxLength={1000} value={payload.mainSanctioningFeeOverrideReason} onChange={(event) => setPayload({ ...payload, mainSanctioningFeeOverrideReason: event.target.value })} /></label><label>Main ACC source/reference<input required maxLength={1000} value={payload.mainSanctioningFeeOverrideReference} onChange={(event) => setPayload({ ...payload, mainSanctioningFeeOverrideReference: event.target.value })} /></label></> : null}{requiresSanctioningFeeOverrideEvidence("consolation", payload.consolationSanctioningFeeRateCents) ? <><label>Consolation override reason<input required maxLength={1000} value={payload.consolationSanctioningFeeOverrideReason} onChange={(event) => setPayload({ ...payload, consolationSanctioningFeeOverrideReason: event.target.value })} /></label><label>Consolation ACC source/reference<input required maxLength={1000} value={payload.consolationSanctioningFeeOverrideReference} onChange={(event) => setPayload({ ...payload, consolationSanctioningFeeOverrideReference: event.target.value })} /></label></> : null}</section>
    </div></fieldset>
    {activated ? <SanctioningFeeOverridePanel payload={payload} busy={busy || rateOverrideBusy} onApply={overrideSanctioningFeeRate} /> : null}
    <div className="setup-heading"><div><h2>Tournament events</h2><p>Click Add Main Event to enter its event name, fees, date and time, included items, and Q Pools. Add Consolation or Satellite events when needed. Two-person Traditional Doubles and Canadian Doubles support shared Digital or Paper team scorecards.</p></div><div className="setup-actions"><button type="button" className="secondary" disabled={busy || !!pending || activated || !!counts.main} onClick={() => add("main")}>Add Main Event</button><button type="button" className="secondary" disabled={busy || !!pending || activated || !!counts.consolation} onClick={() => add("consolation")}>Add Consolation Event</button><button type="button" className="secondary" disabled={busy || !!pending || activated || payload.events.length >= 32} onClick={() => add("satellite")}>Add Satellite Event</button></div></div>
