...
export default function SetupClient({ actorId, tournamentId }: { actorId: string; tournamentId: string }) {
  ...
  return <section className="policy-settings setup-workspace"><h2>Tournament details</h2><p className="auth-note">Primary director: {primaryDirectorName}</p>
    ...
    <SanctioningFeePanel mainRateCents={effectiveMainRateCents} consolationRateCents={effectiveConsolationRateCents} runningTotalCents={sanctioningFeeRunningTotalCents} lastUpdatedAt={sanctioningFeeLastUpdatedAt} refreshing={sanctioningFeeRefreshing} busy={busy || !!pending || rateOverrideBusy} canAdjust={!pending} unavailableReason={pending ? "A setup save is waiting to be confirmed. Retry or resolve that save first, then the rate can be adjusted." : null} onRefresh={() => void refreshSanctioningFee()} onApply={overrideSanctioningFeeRate} />
