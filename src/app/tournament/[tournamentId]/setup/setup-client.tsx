...
function blankPayload(choices: SetupOfficialChoices): SetupPayload {
  ...
  return { tournamentName: "", city: "", venue: "", startsAt: "", endsAt: "", timezone, tournamentContactPhone: "", tournamentContactEmail: "", tournamentMailingAddress: "", sanctioningFeeCents: null, officials: [{ profileId: choices.directorProfileId, role: "director" }, ...choices.coDirectorProfileIds.map((profileId) => ({ profileId, role: "co_director" as const }))], events: [] };
}
...
export default function SetupClient({ actorId, tournamentId }: { actorId: string; tournamentId: string }) {
  ...
  const [amendmentEvents, setAmendmentEvents] = useState<SetupEvent[]>([]), [pendingAmendment, setPendingAmendment] = useState<PendingAmendment | null>(null), [primaryDirectorName, setPrimaryDirectorName] = useState(""), [activationStatusAvailable, setActivationStatusAvailable] = useState(true);
  ...
  async function activate(envelope: PendingActivation) {
    ...
    try { const response = await fetch(`/api/v1/tournaments/${tournamentId}/setup/activation`, { method: "POST", credentials: "same-origin", cache: "no-store", headers: { "content-type": "application/json" }, body: JSON.stringify(envelope.request) }); const data: unknown = await response.json().catch(() => null);
      ...
      if (response.status === 409 && isRejectedSetupActivation(data)) { clearPending(activationStorageKey); setPendingActivation(null); await load(); setMessage(data.code === "already_activated" ? "Current events are already finalized for use." : data.code === "missing_tournament_contact" ? "Tournament contact phone and email are required before current events can be finalized for use." : "Current events were not finalized. Review the saved setup and try again."); return; }
  ...
  return <section className="policy-settings setup-workspace"><h2>Tournament details</h2><p className="auth-note">Primary director: {primaryDirectorName}</p>
    <fieldset disabled={busy || !!pending || activated}><div className="setup-grid">
      <label>Tournament name<input required maxLength={200} value={payload.tournamentName} onChange={(e) => setPayload({ ...payload, tournamentName: e.target.value })} /></label><label>City<input required maxLength={160} value={payload.city} onChange={(e) => setPayload({ ...payload, city: e.target.value })} /></label><label>Venue<input required maxLength={240} value={payload.venue} onChange={(e) => setPayload({ ...payload, venue: e.target.value })} /></label><label>Time zone<input required maxLength={128} value={payload.timezone} onChange={(e) => setPayload({ ...payload, timezone: e.target.value })} /></label><label>Starts<input required type="datetime-local" value={payload.startsAt} onChange={(e) => setPayload({ ...payload, startsAt: e.target.value })} /></label><label>Ends<input required type="datetime-local" value={payload.endsAt} onChange={(e) => setPayload({ ...payload, endsAt: e.target.value })} /></label><label>Tournament contact phone<input required type="tel" inputMode="tel" autoComplete="tel" maxLength={40} value={payload.tournamentContactPhone} onChange={(e) => setPayload({ ...payload, tournamentContactPhone: e.target.value })} /></label><label>Tournament contact email<input required type="email" autoComplete="email" maxLength={320} value={payload.tournamentContactEmail} onChange={(e) => setPayload({ ...payload, tournamentContactEmail: e.target.value })} /></label><label className="setup-wide">Tournament mailing address (optional)<textarea aria-describedby="tournament-mailing-address-help" maxLength={500} value={payload.tournamentMailingAddress} onChange={(e) => setPayload({ ...payload, tournamentMailingAddress: e.target.value })} /><span id="tournament-mailing-address-help" className="field-help">Shown to players. Use a P.O. box, club, venue, or other preferred tournament mailing address—never a private home address.</span></label><MoneyInput key={`sanctioning-fee-${payload.sanctioningFeeCents ?? "blank"}`} label="ACC Sanctioning Fee" value={payload.sanctioningFeeCents} onChange={(sanctioningFeeCents) => setPayload({ ...payload, sanctioningFeeCents })} />
    </div></fieldset>
