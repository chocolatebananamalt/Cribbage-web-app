...
export default function SetupClient({ actorId, tournamentId }: { actorId: string; tournamentId: string }) {
  ...
  function startActivation() { if (!revisionId || version < 1 || dirty || busy || pending || pendingActivation || counts.main !== 1) return; if (!window.confirm("Please confirm you intend to open registration for these events. Event details will no longer be editable.")) return; const envelope: PendingActivation = { kind: "tournament-activation", request: { setupRevisionId: revisionId, expectedVersion: version, confirmed: true, idempotencyKey: crypto.randomUUID() } }; if (!storePending(activationStorageKey, envelope)) { setMessage("This browser cannot safely retain the finalization request for recovery."); return; } void activate(envelope); }
