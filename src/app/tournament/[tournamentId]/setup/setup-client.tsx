...
export default function SetupClient({ actorId, tournamentId }: { actorId: string; tournamentId: string }) {
  ...
  return <section className="policy-settings setup-workspace"><h2>Tournament details</h2><p className="auth-note">Primary director: {primaryDirectorName}</p>
    <fieldset disabled={busy || !!pending || activated}><div className="setup-grid setup-details-grid">
      ...
      <fieldset className="setup-director-info setup-span-12"><legend>Tournament Director Information (shown to players)</legend><p className="field-help setup-director-help">Use the information players should see for this tournament. It is separate from private account information.</p><label className="setup-director-first">First name<input required maxLength={80} autoComplete="given-name" value={payload.tournamentDirectorFirstName} onChange={(e) => { const tournamentDirectorFirstName = e.target.value; setPayload({ ...payload, tournamentDirectorFirstName, tournamentDirectorPublicName: directorProjection(tournamentDirectorFirstName, payload.tournamentDirectorLastName) }); }} /></label><label className="setup-director-last">Last name<input required maxLength={80} autoComplete="family-name" value={payload.tournamentDirectorLastName} onChange={(e) => { const tournamentDirectorLastName = e.target.value; setPayload({ ...payload, tournamentDirectorLastName, tournamentDirectorPublicName: directorProjection(payload.tournamentDirectorFirstName, tournamentDirectorLastName) }); }} /></label><label className="setup-director-phone">Phone<input type="tel" inputMode="tel" autoComplete="tel" maxLength={40} value={payload.tournamentContactPhone} onChange={(e) => setPayload({ ...payload, tournamentContactPhone: e.target.value })} /></label><label className="setup-director-email">Email<input type="email" autoComplete="email" maxLength={320} value={payload.tournamentContactEmail} onChange={(e) => setPayload({ ...payload, tournamentContactEmail: e.target.value })} /></label><label className="setup-director-mailing">Mailing Address for Correspondence<span id="tournament-mailing-address-help" className="field-help">*Note: This address will be visible to players.</span><textarea aria-describedby="tournament-mailing-address-help" maxLength={500} value={payload.tournamentMailingAddress} onChange={(e) => setPayload({ ...payload, tournamentMailingAddress: e.target.value })} /></label></fieldset>
    </div></fieldset>
