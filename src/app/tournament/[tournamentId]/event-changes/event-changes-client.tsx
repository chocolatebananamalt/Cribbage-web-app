...
export default function EventChangesClient({ tournamentId }: { tournamentId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setWorkspace(data); setMessage(data.canManage
      ? "Only the primary director may make these exceptional changes. Enter a reason to enable them."
      // Both branches of this ternary used to carry the same sentence, so a
      // co-director read a neutral policy note, saw no controls, and had
      // nothing telling them the screen was closed to them personally.
      : "You cannot make these changes. Only the primary director who created this tournament may retire or replace an event; a co-director cannot. Ask them to make the change.");
  }, [tournamentId]);
  useEffect(() => {
  ...
  return <section className="setup-amendment"><p className="auth-note">Cancel/Retire and Cancel/Replace preserve event, roster, payment, and audit history. They are unavailable after Start Play. Co-directors cannot use them.</p>
    ...
    </li>)}</ul>{pendingChange ? <section className="setup-amendment" role="alertdialog" aria-modal="true" aria-labelledby="event-change-confirmation-title"><h2 id="event-change-confirmation-title">{pendingChange.action === "retire" ? `Retire ${pendingChange.event.name}?` : `Cancel and replace ${pendingChange.event.name}?`}</h2><p>{pendingChange.action === "retire" ? `${pendingChange.event.name} keeps its registrations, payment history and audit history, and stops taking new enrollment. Its ${pendingChange.event.participantCount} enrolled participant${pendingChange.event.participantCount === 1 ? "" : "s"} stay on record.` : `${pendingChange.event.name} remains in history. Its eligible enrollments and tournament-level payment credit references will be copied to a separately identified replacement for your review.`}</p><div className="setup-actions"><button type="button" className="secondary" disabled={busy} onClick={() => setPendingChange(null)}>Cancel</button><button type="button" className={pendingChange.action === "retire" ? "secondary danger-button" : "primary"} disabled={busy} onClick={() => void change(pendingChange.event, pendingChange.action)}>{pendingChange.action === "retire" ? "Yes, Retire Event" : "Yes, Cancel & Replace"}</button></div></section> : null}
  </section>;
}
