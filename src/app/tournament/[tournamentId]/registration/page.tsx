...
export default async function RegistrationLinkPage({ params }: { params: Promise<{ tournamentId: string }> }) {
  ...
  return <main className="auth-shell"><section className="auth-card corrections-card" aria-labelledby="registration-link-title"><p className="eyebrow">OPERATIONS</p><h1 id="registration-link-title">Registration Link</h1><p className="auth-note">Create or manage one QR-ready link for this tournament. Newly issued or replacement links can be viewed again by authorized officials; a legacy active link remains usable but cannot be shown again until it is replaced.</p><Link className="guide-link" href={`/tournament/${tournamentId}`}>Back to tournament</Link><RegistrationLinkClient tournamentId={tournamentId} initialState={state} /><SharedDeviceSignOut /></section></main>;
}
