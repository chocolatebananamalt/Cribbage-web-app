...
const normalizeAccInput = (value: string) => value.toUpperCase().replace(/[^A-Z0-9]/g, "");
...
export default function RosterClient({ actorId, tournamentId, rosterEntries, withdrawnRosterEntries, promotionCandidates }: { actorId: string; tournamentId: string; rosterEntries: RosterEntry[]; withdrawnRosterEntries: RosterEntry[]; promotionCandidates: PromotionCandidate[] }) {
  ...
  void locked;
  const [reasonCode, setReasonCode] = useState<RosterStatusRequest["reasonCode"]>("duplicate_entry"), [note, setNote] = useState("");
  ...
  async function setRosterStatus(entry: RosterEntry, action: "withdraw" | "reinstate") {
    ...
    try { const response = await send("roster-status", body), payload: unknown = await response.json().catch(() => null);
      if (response.ok && isAcceptedRosterStatus(payload, body)) { setResolveTarget(null); setNote(""); setMessage(action === "withdraw" ? "The player was removed from active roster operations. History is preserved." : "The roster identity was reinstated."); router.refresh(); return; }
