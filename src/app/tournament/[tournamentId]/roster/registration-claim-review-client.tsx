...
export default function RegistrationClaimReviewClient({ actorId, tournamentId, claims }: { actorId: string; tournamentId: string; claims: RegistrationClaimReviewItem[] }) {
  ...
  return <section className="registration-claim-review" aria-labelledby="claim-review-title">
    ...
    {pending.length ? <ul className="correction-list">{pending.map((claim) => { const draft = draftFor(claim.claimId); const claimLocked = locked?.claimId === claim.claimId; return <li className="correction-item" key={claim.claimId}>
      <p><strong>{claim.displayName}</strong> · {claim.email}{claim.accNumber ? ` · ACC # ${claim.accNumber}` : ""}</p>
      <p className="auth-note">Scorecard: {claim.scorecardType === "digital" ? "Digital" : "Paper"} · Planned payment: {claim.intendedPaymentMethod} · Submitted <time dateTime={claim.submittedAt}>{formatUtcDateTime(claim.submittedAt)}</time></p>
      {claim.requiresDistinctConfirmation ? <><p className="auth-note"><strong>Possible duplicate—director review required.</strong> This is a name or email match only. A matching ACC # or the exact same first name, last name, and email remains blocked.</p><label className="publication-confirmation"><input type="checkbox" checked={draft.confirmedDistinct} disabled={busy || !!locked} onChange={(event) => updateDraft(claim.claimId, { confirmedDistinct: event.target.checked })} /> I reviewed the possible match and confirmed this is a different person.</label></> : null}
      ...
      <div className="correction-actions"><button className="primary" type="button" disabled={!ready || busy || !!locked || (claim.requiresDistinctConfirmation && !draft.confirmedDistinct)} onClick={() => decide(claim, "approved_for_roster")}>{claimLocked && locked?.decision === "approved_for_roster" ? "Retry approval" : claim.requiresDistinctConfirmation ? "Different person — approve for roster" : "Approve for roster"}</button><button className="secondary" type="button" disabled={!ready || busy || !!locked} onClick={() => decide(claim, "rejected")}>{claimLocked && locked?.decision === "rejected" ? "Retry rejection" : claim.requiresDistinctConfirmation ? "Same person — reject duplicate" : "Reject request"}</button></div>
