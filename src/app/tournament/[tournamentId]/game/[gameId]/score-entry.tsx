...
import { JudgeCallButton } from "./judge-call-button";
...
function publicMessage(status: number, payload: unknown) {
  ...
  if (code === "future_game_locked") return "This upcoming game opens after your earlier scheduled game is verified.";
  if (code === "event_play_paused") return "The director has paused play. Your entry is kept on this device. Submit it again once play resumes.";
  if (code === "event_play_closed") return "The director has closed play for this event, so no new score can be entered. Speak to the director if this game still needs a result.";
...
export function LiveScoreEntry({ context }: { context: AssignedGameContext }) {
  ...
  const [offlineReplayClosed, setOfflineReplayClosed] = useState(false);
  ...
  const syncOffline = async (record: OfflineQueueRecord) => {
    if (offlineSyncInFlight.current || !navigator.onLine) return;
    ...
    try {
      ...
      if (response.ok || response.status === 409) {
        ...
        const disposition = (payload as { disposition: "accepted" | "rejected" | "quarantined" | "conflict" }).disposition;
        if ((response.ok && disposition !== "accepted") || (response.status === 409 && disposition === "accepted")) {
          setStatus("The sync response contradicted its receipt. Your saved offline entry remains on this device.");
          return;
        }
      ...
      if (!response.ok) {
        ...
        else if (response.status === 423) setStatus("The director has paused or closed play, so this score was not sent. Your saved entry is still on this device and will sync once play resumes.");
  ...
  const key = (value: (typeof keypad)[number]) => {
    if (busy || submissionId || pendingSubmission || offlineReplayClosed) return;
  ...
  const submit = async () => {
    if (busy || submissionId || offlineReplayClosed || !hydrated) return;
  ...
  return <main className="auth-shell"><section className="auth-card live-score" aria-labelledby="live-score-title"><p className="eyebrow">SCORE ENTRY</p><h1 id="live-score-title">Current Game Results</h1><p className="auth-note">{context.eventName} · Game {context.roundNumber}</p><JudgeCallButton tournamentId={context.tournamentId} gameId={context.gameId} /><p className="auth-note" role="status">{offlinePageReady ? "Offline Ready" : online ? "Preparing Offline Use…" : "Offline reload is not ready on this device."}</p><div className="live-matchup"><strong>{context.player.displayName} <em>(ID#: {context.player.verificationId})</em></strong><span>Table/Seat {context.player.tableSeat}</span><b>VS</b><strong>{context.opponent.displayName} <em>(ID#: {context.opponent.verificationId})</em></strong><span>Table/Seat {context.opponent.tableSeat}</span></div><fieldset><legend>Game Winner:</legend><button type="button" className={winner === "player" ? "pick selected" : "pick"} disabled={busy || !!submissionId || !!pendingSubmission || !!offlineRecord || offlineReplayClosed} aria-pressed={winner === "player"} onClick={() => setWinner("player")}>{context.player.displayName} won</button><button type="button" className={winner === "opponent" ? "pick selected" : "pick"} disabled={busy || !!submissionId || !!pendingSubmission || !!offlineRecord || offlineReplayClosed} aria-pressed={winner === "opponent"} onClick={() => setWinner("opponent")}>{context.opponent.displayName} won</button></fieldset><div className="entry"><span>Spread Points:</span><output className={derived ? "number" : "number invalid"}>{marginText || "·"}</output><SkunkAid level={derived?.skunkLevel ?? 0} /></div><div className="keypad" aria-label="Spread points keypad">{keypad.map((value) => <button type="button" key={value} onClick={() => key(value)} disabled={busy || !!submissionId || !!pendingSubmission || !!offlineRecord || offlineReplayClosed}>{value === "clear" ? "Clear" : value === "backspace" ? "⌫" : value}</button>)}</div>{derived ? <ResultPreview context={context} score={derived} /> : null}<p className="live-status" role="status">{status}</p>{offlineRecord ? <button type="button" className="primary full" disabled={busy || !online} onClick={() => syncOffline(offlineRecord)}>{busy ? "Syncing…" : online ? "Sync Saved Entry" : "Waiting for Connection"}</button> : !submissionId && !offlineReplayClosed ? pendingSubmission ? <button type="button" className="primary full" disabled={busy || !hydrated} onClick={submit}>{busy ? "Submitting…" : "Retry This Same Entry"}</button> : <button type="button" className="primary full" disabled={!derived || busy || !hydrated} onClick={() => setReviewing(true)}>Review Result</button> : canConfirm ? <button type="button" className="primary full" disabled={busy} onClick={confirm}>{busy ? "Confirming…" : "Confirm My Entry"}</button> : null}<PaperDigitalGuide /><Link className="guide-link" href={`/tournament/${context.tournamentId}/scorecard?event=${context.eventId}`}>View Scorecard</Link><Link className="guide-link" href={`/tournament/${context.tournamentId}/how-to`}>Open Start Here / How To</Link><SharedDeviceSignOut /></section></main>;
}
