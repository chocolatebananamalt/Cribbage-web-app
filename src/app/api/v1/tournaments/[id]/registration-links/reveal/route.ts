...
import { revealRegistrationLink } from "../../../../../../../lib/registration-link-reveal";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withApiFailureBoundary(async () => {
    ...
    const result = await revealRegistrationLink(createServerOnlyAdminClient(), {
      actorId: subject, tournamentId: id, expectedLinkId: body.expectedLinkId,
      expectedVersion: body.expectedVersion, operationId: body.operationId,
    });
    if (result.status === "legacy_unrecoverable") return apiJson({ error: "legacy_credential_unavailable" }, { status: 409 });
    if (result.status !== "revealable") return apiJson({ error: "registration_link_conflict" }, { status: 409 });
    return apiJson({ status: "revealable", credential: result.credential });
  });
}
