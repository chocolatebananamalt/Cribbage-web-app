...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isUuid(id) || !isPaymentVoidRequest(body)) return NextResponse.json({ error: "invalid_payment_void" }, { status: 400 });
  ...
  if (isVoidedPayment(data, body)) return NextResponse.json(data, { headers: { "cache-control": "private, no-store" } });
