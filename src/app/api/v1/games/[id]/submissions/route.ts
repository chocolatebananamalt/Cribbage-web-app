...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isUuid(id) || !isUuid(body.submissionId) || !isUuid(body.idempotencyKey) || ![1, 2].includes(body.submissionSlot as number) || !["a", "b"].includes(body.winnerSide as string) || !Number.isInteger(body.margin) || (body.margin as number) < 1 || (body.margin as number) > 121) return NextResponse.json({ error: "invalid_submission" }, { status: 400 });
