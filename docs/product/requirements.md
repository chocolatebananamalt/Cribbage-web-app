...
- **Set Up Tournament** is the single canonical source for tournament, director, venue, event, fee, Q-pool, and payout information. It offers either guided entry based on the observed ACC sanctioning-request fields or an existing-flyer upload for assisted extraction followed by director confirmation. That approved data feeds Events and Flyer, seating, results, finance, and a director-assisted draft worksheet aligned to observed ACC fields; it is not an ACC-approved import or submission contract. It must not submit into the ACC portal unless the ACC provides a documented, authorized API or import contract.
