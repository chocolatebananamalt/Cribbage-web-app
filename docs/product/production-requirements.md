...
The state machines define product integrity, not invented ACC policy. The source-defined post-lunch, late-player, first-sit-out makeup, early-departure, two-games-per-opponent, final-game substitute, excluded-extra-game, and playoff-absence outcomes above are approved implementation inputs. Mixed-up rotation remains an audited director choice. Family restrictions not stated in the cached sources, any unverified organization-specific rotation variant, Q-pool payout/rounding, current MRP tables/byes, and any ACC import/API contract remain separate unresolved items; they do not block implementing the defined tournament-operation cases.
...
- Cash App Pay, Apple Pay, Google Pay, Venmo, and Venmo Tap to Pay are future optional integrations. Every method is default-off and MUST remain unavailable until its merchant/account ownership, credentials, webhook authenticity, duplicate/out-of-order events, cancellation, refund/chargeback, settlement, and live provider tests pass. Missing digital-payment access MUST NOT block the October cash/check workflow.
...
The owner approved retained paper-scorecard images on 2026-09-12. Originals MUST use a private restricted-hold bucket with opaque object paths, no direct browser storage policy, server-authorized signed upload, immutable upload/access receipts, declared content-type and size limits, and post-upload byte-count plus SHA-256 verification. OCR output MUST be an editable, versioned, non-authoritative draft requiring human review. Impossible spreads, ambiguous plus/minus cells, duplicate games, or any model claim of authority MUST fail closed. External OCR remains default-off until a real provider probe and false-read evidence pass; manual paper transcription remains usable without it.
