...
|---|---|---|---|
| R-REG-01 | Registration, check-in, roster identity, shared-device clearing, and seating are server-scoped workflows | director creates/imports roster, checks in player, assigns Table/Seat, and a cleared shared device starts a new context | unauthorized role, duplicate identity, stale shared-device context, or cross-tournament seating is rejected |
...
| R-SCORE-01 | Margin, per-card lines, canonical match linkage, and derived scoring follow the scorecard rules | fixture records reciprocal plus/minus and 0/2/3 points | invalid margin, conflicting match/card linkage, or client summary is rejected |
...
- **Seating/rotation:** the server assigns a unique Table/Seat value per round and records effective time and source. Manual overrides require a director reason. Rotation, anchors, sit-outs, family restrictions, lateness/forfeits, and any seating eligibility rule MUST be backed by an approved dated ACC fixture before an official schedule or export is produced.
...
- The digital card MUST mirror the paper layout: game number, derived game points, plus, minus, opponent first and last name, and Table/Seat (for example `A-7`), followed by totals, games won/lost, and net plus/minus. Opponent initials are removed.
- Every card line MUST link to a canonical match/game record and contain the player/card side, opponent/card side, round, and event identifiers. A match may have two reciprocal card-line projections, but neither may be flattened into one margin-only record. This linkage is required to apply ACC Rule 12.2 correction cases atomically to both affected cards and all derived totals.
- Required Rule 12.2 fixture coverage is the source's cases (a)–(i). Each fixture MUST assert the resulting state of both cards, canonical match linkage, totals, standings inputs, and audit trail; do not implement from a single-margin fixture or infer an additional ACC rule from a prototype.
