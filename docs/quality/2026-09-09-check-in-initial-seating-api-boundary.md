...
- No authenticated director/co-director browser session, independent-session authorization test, real roster/check-in lifecycle, or deployed route smoke test was run in this pass.
- No director-facing check-in/seating interface, print view, player account linking/delivery, initial assignment generator, late-entry policy, round rotation, or table-playthrough guidance is provided by these routes. Those remain release gates.
