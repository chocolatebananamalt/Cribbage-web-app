...
`canonical_game` is the single identity for one played game between two card sides. Each side has its own `card_scoreline` linked to the same game, including game points, plus, minus, player, opponent, Table/Seat, and source/method. Reciprocal lines are projections of the same canonical game; they are never flattened into a single margin-only row. Corrections update **all affected scorelines** and all affected derived totals atomically while preserving the original verification/audit record (`R-SCORE-01`, `R-CORR-01`).
