...
| The owner-approved disposable Supabase project is reconstructed through migration 0152 with the same pilot catalog counts and no unindexed foreign keys. | Hosted restore evidence, 2026-09-12 | Use it for isolated synthetic validation. Do not copy private pilot records into it without an approved backup/restore rehearsal and evidence-handling plan. |
...
| A stored paper-card photo is useful only if a later independent official can reopen it during cross-checking. | Implementation reconciliation, 2026-09-13 | Stream integrity-verified bytes through an authenticated no-store app route, never a public object URL. Require a current independent official identity, deny the uploader and game participants, and append every accepted read to the storage-access ledger. |
