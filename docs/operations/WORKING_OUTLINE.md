...
| 2 | **Platform and access foundation.** Vercel Production, Supabase Auth/database, private role boundaries, audit receipts, and safe registration links operate. | Cross-checker assignment implemented and pilot migration verified; deployment and acceptance proof remain | **Implementation 2026-09-12; acceptance target 2026-09-16** | Production hosting, passwordless sign-in, named tournament chooser, registration link/claim/review, witnessed roster-account activation, and protected role boundaries operate. Migration 0161 adds an audited director workflow to assign already-linked accounts as cross-checkers; rollback-only verification passes on disposable and pilot databases and both RPCs are service-role-only. Deployment plus a genuinely independent account/session rehearsal remain. |
...
1. Deploy the protected Cross-checker Assignments screen, then complete the
   actual October pilot roster and link at least two independent
