...
| 4 | **Hybrid, paper-card, and durable offline workflow.** Shared/dead-phone handling; paper/digital guidance; authenticated offline score capture and later server reconciliation; restricted paper-card camera capture, human-reviewed transcription, comparison, and exception-only cross-check queue. | In progress | **Estimated 2026-10-16** | Offline score capture is mandatory for the first pilot, superseding the earlier connected-first scope. The queue contract exists, but encrypted/auth-bound storage, replay, conflict handling, and real two-device reconnect proof do not. Paper-card capture and verification contracts exist, but the connected camera/storage/review flow and real card tests also remain. |
...
3. Complete durable offline score capture/reconnect and connected paper/digital
   operation before online payment processing or other conveniences.
