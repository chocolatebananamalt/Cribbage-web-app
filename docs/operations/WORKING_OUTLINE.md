...
| 3 | **Tournament setup, players, check-in, seating, and directory.** Main/Consolation/Satellites, supported doubles, game counts, fees, Q-pools, roster/import, payment status, check-in, closure, capacity, assignments, scoped lookup, event participants, and schedules operate under one tournament. | Deployed; rehearsal remains | **Deployed 2026-09-17; rehearsal target 2026-09-17** | Manual/CSV/public intake, registration freeze, dynamic table plan, printable seating, audited schedule publication/amendment, captain-selected shared team mode, team claims, scorer designation/linkage, published starting/current Table/Seat and permanent IDs, participant directory with private-field filtering, and printable paper-card lists are live. The Production event-specific Day-of-play Event QR Check-In workspace adds a director-controlled 60-second rotating, opaque check-in code for each event and a desk alternative that independently enforces payment and event enrollment. A valid scan safely grants a separate opaque five-minute required-identity completion session. Consolation cannot open before Main qualification finalizes. One physical registration-close → seating → directory → event QR rotation/expiry → schedule rehearsal remains; email delivery is fail-closed pending ACC-controlled SMTP configuration and load testing. |
