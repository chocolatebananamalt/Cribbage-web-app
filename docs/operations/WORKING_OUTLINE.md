...
| 2 | **Platform and access foundation.** Vercel Production, Supabase Auth/database, private role boundaries, audit receipts, and safe registration links operate. | Implementation complete; acceptance proof remains | **Implemented 2026-09-12; acceptance target 2026-09-16** | Production hosting, passwordless sign-in, named tournament chooser, registration link/claim/review, audited role boundaries, migrations through 0153, rollback, and runtime-error checks pass. A genuinely independent account/session rehearsal remains. |
...
| 8 | **Deferred full suite.** Production Rulebook/quick-reference integration, Judge Desk, digital team scoring, flyer creation/import, online payments, SMS, OCR, and automatic ACC submission. | Planned; provider preflight implemented | **After 2026-10-03** | These do not block the Standard Singles pilot. Payment, SMS, and OCR now have executable default-off configuration checks plus exact external dependencies in `PROVIDER_ACTIVATION_PLAN.md`; configuration alone never activates them. October uses audited manual payment, printed seating, and human paper-card entry. |
