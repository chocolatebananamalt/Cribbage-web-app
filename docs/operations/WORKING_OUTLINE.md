...
| 2 | **Secure platform foundation.** Protected Vercel Preview, private Supabase schema/RLS boundaries, server-only routes, audit receipts, and safe registration-link lifecycle exist. | In progress | **Estimated 2026-09-17** | Registration issue/rotate/close protections and atomic registration close are deployed to the pilot. The source now also has a role-gated QR-registration workspace that renders the one-time fragment link locally without browser persistence; director link management and public claims use distinct default-off release gates. The protected Seating workspace reads the authoritative closure state and cannot publish initial seating until a director closes registration. The deployed game page fails closed with a non-actionable notice when its database contract is incompatible, instead of presenting score controls. **Critical containment item:** the shared pilot still grants authenticated execution on seven obsolete Rule 12 correction RPCs because source revocations `0096`–`0098` are not yet applied; it must be contained before any role fixture or live pilot use. Real races, independent sessions, browser proof, hosted auth-provider confirmation, and reviewed database migration parity remain. |
...
| 4 | **Hybrid, offline, and paper-card workflow.** Auth-bound offline queue/replay; shared/dead-phone handling; paper/digital guidance; restricted paper-card camera capture, human-reviewed transcription, comparison, and exception-only cross-check queue. | In progress | **Estimated 2026-10-16** | A strict, unconnected offline-queue contract now prevents fake local verification, sensitive queue fields, expired capabilities, and unbound deletion receipts. It is not an IndexedDB adapter or replay service. Paper-card capture contract is approved as a requirement. Still requires a server replay transaction, non-exportable device/session key, OCR/storage choice, retention approval, restricted access policies, real camera tests, and no automatic verification. |
...
| 6 | **Release operations and accessibility.** Separate staging/production configuration, monitoring, alerts, rollback and restore drills, security review, phone/desktop/zoom accessibility evidence, support/incident procedures, and an enforced merge/release gate. | Not started | **Estimated 2026-11-06** | Hosting projects exist. Production secrets, backups/restores, monitoring, custom domain, a release runbook, and a private-repository merge guardrail still need direct proof. |
