...
| 2 | **Secure platform foundation.** Protected Vercel Preview, private Supabase schema/RLS boundaries, server-only routes, audit receipts, and safe registration-link lifecycle exist. | In progress | **Estimated 2026-09-17** | Registration issue/rotate/close protections and atomic registration close are deployed to the pilot. Real disposable proof now covers active-link/no-link closure, exact retry, ID-reuse rejection, and browser-role denial. Still needs real races, director UI, independent sessions, and hosted auth-provider confirmation. |
| 3 | **Core live tournament workflow.** Director setup, registration/check-in, closure, seating and table-plan exceptions, event enrollment, two-player independent scoring/confirmation, corrections, judge/cross-check queues, and official ACC fixtures operate server-authoritatively. | In progress | **Estimated 2026-09-30** | Foundations exist for setup, check-in, seating, enrollment, scores, and corrections. Missing operational completion, full official fixtures, real multi-user execution, and the director-facing registration-close control. |
...
2. Finish the director-facing registration and closure workflow, with real
   independent-session browser evidence.
